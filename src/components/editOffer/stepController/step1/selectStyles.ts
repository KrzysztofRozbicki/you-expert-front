export default (isError: boolean): any => ({
  container: (styles) => ({ ...styles, fontSize: '0.8rem' }),
  control: (styles, info) => {
    const { hasValue, menuIsOpen } = info;
    return {
      ...styles,
      width: '100%',
      height: '56px',
      ...
      padding: '7px',
      backgroundColor: '#fff',
      border: `1px solid ${isError ? '#D74F3E' : '#DCDCF4'}`,
      color: '#020055',
      borderRadius: '55px',
      boxShadow: 'none',
      zIndex: menuIsOpen ? '2' : '0'
    };
  },
